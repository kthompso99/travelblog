...
- **Audit system (first-class infrastructure):** Content quality management with AI scoring, WebSocket dashboard, and CLI tools. Core files: `scripts/audit/audit-shared.js` (shared logic, prompts, scoring), `scripts/audit/audit-server.js` (WebSocket dashboard on port 3001), `scripts/audit/anthropic-audit.js` + `gpt-audit.js` (CLI audit runners), `scripts/audit/trip-audit.js` + `trip-audit-api.js` (trip-level audits), `scripts/audit/audit-status.js` (readiness tracking: 8.5 article / 8.7 trip thresholds), `scripts/audit/audit-rankings.js` (score-based rankings), `scripts/audit/audit-history.js` (historical trends). Usage: `npm run opus-audit -- <trip>/<file>`, `npm run dashboard` (audit-runner.html). Legacy experiments in `scripts/audit-legacy/`.
- **No self-spawning (CRITICAL):** NEVER use `spawn()` or `execSync()` to invoke own JavaScript code via npm scripts. Pattern: All scripts export core logic, CLI and API both import and call the same functions directly. Only spawn for external tools (git, ImageMagick, etc.). Structure: `export async function coreLogic(params) { ... }` at top, CLI wrapper with execution guard `if (process.argv[1] === fileURLToPath(import.meta.url))` at bottom. Result: Zero process overhead, single source of truth, shared implementation.
...
- **Maps:** Two Google Maps instances — global (`/map/`) and per-trip (`/trips/{slug}/route.html`). Trip intro pages show a hero image, not a map. Amber SVG divIcon markers. Popup hover-linger: 300 ms delay on mouseout, cancelled by mouseenter on popup. Nav z-index must stay ≥ 2000.
