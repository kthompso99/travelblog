...
function generateTripLocationPage(tripMetadata, location, allLocations, locationIndex, config, domain) {
    ...
    const html = baseTemplate
        ...
        .replace(/{{TRIPS_MENU}}/g, tripsMenu)
        .replace(/{{TRIP_SUBMENU}}/g, tripSubmenu)
        .replace(/{{PRE_MAIN}}/g, buildTripHero(tripMetadata))
...
function generateTripMapPage(tripMetadata, allContent, config, domain) {
    ...
    const locationsForMap = locations.map(loc => ({
        ...loc,
        slug: loc.title.toLowerCase().replace(/\s+/g, '-')
    }));
    ...
    locations.forEach((loc, idx) => {
        const locationSlug = loc.title.toLowerCase().replace(/\s+/g, '-');
    ...
    const firstSlug = firstItem ? firstItem.title.toLowerCase().replace(/\s+/g, '-') : '';
    ...
    const html = baseTemplate
        ...
        .replace(/{{TRIP_SUBMENU}}/g, tripSubmenu)
        .replace(/{{PRE_MAIN}}/g, buildTripHero(tripMetadata))
        .replace(/{{CONTENT}}/g, mapPageContent)
        .replace(/{{SCRIPTS}}/g, '');
