...
<script>
    let map = null;
    let markers = [];
    const config = ${JSON.stringify(mapConfig, null, 2)};
    const APP_BASE = '/';
        ...
        google.maps.event.addListener(tripInfoWindow, 'domready', () => {
            ...
            if (iwOuter) {
                ...
                iwOuter.addEventListener('mouseleave', () => {
                    ...
                    tripHoverTimeout = setTimeout(() => tripInfoWindow.close(), 300);
                });
            }
        });
        ...
        google.maps.event.addListener(locationInfoWindow, 'domready', () => {
            ...
            if (iwOuter) {
                ...
                iwOuter.addEventListener('mouseleave', () => {
                    ...
                    locationHoverTimeout = setTimeout(() => locationInfoWindow.close(), 300);
                });
            }
        });
            ...
            if (dest.locations && dest.locations.length > 0) {
                dest.locations.forEach((loc, idx) => {
                    ...
                    const thumbHtml = loc.thumbnail
                        ? '<img src="../trips/' + dest.slug + '/' + loc.thumbnail + '" alt="' + loc.name + '" class="marker-popup-image" onerror="this.remove()">'
                        : '';
...
function generateTripLocationPage(tripMetadata, location, allLocations, locationIndex, config, domain) {
    ...
    let metadataHtml = '';
    // Metadata display removed per user preference
    // if (location.place) {
