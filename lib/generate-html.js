...
function buildTripsMenu(trips, basePath = '/') {
    ...
    Object.keys(continents).sort().forEach(continent => {
        ...
        continents[continent].forEach(trip => {
            html += `
                                <a href="${basePath}trips/${trip.slug}/">${escapeHtml(trip.title)}</a>`;
        });
...
function buildTripGrid(trips, basePath = '/') {
    ...
    trips.forEach(trip => {
        const thumbnailUrl = trip.thumbnail || `https://via.placeholder.com/280x200?text=${encodeURIComponent(trip.title)}`;
