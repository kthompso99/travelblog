...
function buildTripGrid(trips, basePath = './') {
    ...
    trips.forEach(trip => {
        ...
        html += `
        <a href="${basePath}trips/${trip.slug}/" class="destination-card" data-trip-id="${trip.id}" data-continent="${escapeHtml(continent)}" data-country="${escapeHtml(country)}" data-year="${year}" data-title="${escapeHtml(trip.title)}">
...
function buildPrevNextNav(allContent, currentIndex, basePath = './') {
    ...
    // Previous link
    if (currentIndex === 0) {
        // First content item -> previous is intro page
        html += `
            <a href="${basePath}" class="prev-link">← Previous: Overview</a>`;
    } else if (currentIndex > 0) {
        // Other items -> previous is the item before this one
        const prevItem = allContent[currentIndex - 1];
        const prevSlug = prevItem.title.toLowerCase().replace(/\s+/g, '-');
        html += `
    ...
    if (currentIndex < allContent.length - 1) {
        ...
        const nextSlug = nextItem.title.toLowerCase().replace(/\s+/g, '-');
        html += `
...
function buildIntroPageNextLink(allContent, basePath = './') {
    ...
    const firstSlug = firstItem.title.toLowerCase().replace(/\s+/g, '-');

    return `
        <div class="prev-next-links">
            <a href="${basePath}${firstSlug}.html" class="next-link">Next: ${escapeHtml(firstItem.title)} →</a>
        </div>`;
}
...
function generateTripIntroPage(tripMetadata, allContent, config, domain) {
    ...
    // Build map section HTML (only if there are locations)
    const mapSectionHtml = locations.length > 0 ? `
    <div id="map" class="trip-bottom-section">
        <div class="trip-map-container">
            <h2>Trip Map</h2>
            ${mapHtml}
        </div>
        <div class="trip-toc-container">
            <h2>Locations</h2>
            ${tocHtml}
        </div>
    </div>` : '';
...
function generateTripArticlePage(tripMetadata, article, allContent, articleIndex, config, domain) {
    ...
    const articlePageContent = articleTemplate
        ...
        .replace(/{{LOCATION_METADATA}}/g, metadataHtml)
        .replace(/{{LOCATION_CONTENT}}/g, article.contentHtml || '<p>Content not available</p>')
        .replace(/{{PREV_NEXT_NAV}}/g, prevNextHtml);
    ...
    const html = baseTemplate
        ...
        .replace(/{{TRIP_SUBMENU}}/g, tripSubmenu)
        .replace(/{{PRE_MAIN}}/g, '')
        .replace(/{{CONTENT}}/g, articlePageContent)
        .replace(/{{SCRIPTS}}/g, '');
