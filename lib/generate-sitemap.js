...
function generateSitemap(trips, domain) {
    ...
    trips.forEach(trip => {
        ...
        urls.push({
            ...
            lastmod: trip.endDate || trip.beginDate || null,
            changefreq: 'yearly',
            priority: '0.8'
        });

        // Individual content pages (locations and articles)
        if (trip.content && trip.content.length > 0) {
            trip.content.forEach(contentItem => {
                const itemSlug = getContentItemSlug(contentItem);
                urls.push({
                    loc: `${domain}/trips/${trip.slug}/${itemSlug}.html`,
                    lastmod: trip.endDate || trip.beginDate || null,
                    changefreq: 'yearly',
                    priority: '0.7'
                });
