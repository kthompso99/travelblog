...
import { generateHomeMetadata, escapeHtml } from './seo-metadata.js';
...
function generateHomepage(config, domain) {
    ...
    // Intro section: photo + text side-by-side (or text-only fallback)
    let introHtml = '';
    if (config.site.intro) {
        const introText = escapeHtml(config.site.intro)
            .replace('great journeys combine comfort, culture, adventure, and history.', '<strong>great journeys combine comfort, culture, adventure, and history.</strong>');
        const introPhoto = config.site.introPhoto
            ? `<div class="intro-photo-wrapper"><img src="${config.site.introPhoto}" alt="Kevin and Tiffany" class="intro-photo"></div>`
            : '';
        const introCta = introPhoto
            ? '<a href="./about/" class="intro-cta">Learn More About Us \u2192</a>'
            : '';
        introHtml = introPhoto
            ? `<div class="intro-section">${introPhoto}<div class="intro-text"><p class="hero-intro">${introText}</p>${introCta}</div></div>`
