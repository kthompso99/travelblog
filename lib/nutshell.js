...
import { NUTSHELL_MARKER_START, NUTSHELL_MARKER_END, NUTSHELL_FIELDS } from './constants.js';
...
const DURATION_SVG_ATTRS = 'viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"';
const DURATION_ICON_CLOCK    = `<svg ${DURATION_SVG_ATTRS}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>`;
const DURATION_ICON_CALENDAR = `<svg ${DURATION_SVG_ATTRS}><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>`;

/** Pick a duration icon based on the unit (clock for hours/minutes, calendar for days+). */
function getDurationIcon(duration) {
    const lower = duration.toLowerCase();
    return (lower.includes('hour') || lower.includes('min'))
        ? DURATION_ICON_CLOCK
        : DURATION_ICON_CALENDAR;
}
...
function renderNutshell(parsed) {
    ...
    const durationHtml = duration
        ? `<span class="nutshell-duration">${getDurationIcon(duration)} ${duration}</span>`
        : '';

    return `<section class="nutshell-section">
<h2>🥜 ${name} in a Nutshell</h2>
<div class="nutshell-meta"><div class="nutshell-subtitle">Two Travel Nuts Verdict</div><div class="nutshell-badges">${durationHtml}${verdictHtml}</div></div>
...
export { parseNutshellBlock, renderNutshell, processNutshell };
