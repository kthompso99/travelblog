...
import { generateTripMetadata, generateContentPageMetadata, escapeHtml } from './seo-metadata.js';
import { getContentItemSlug } from './slug-utilities.js';
import { getTripImagePath } from './image-utilities.js';
import { readTemplate, formatMonthYear, renderPage } from './generate-html-helpers.js';
import { fillTemplate } from './template-utilities.js';
import { buildCommentPageId, buildCommentsSection } from './remark42-config.js';

/**
 * Floating table of contents script (injected on intro and content pages).
 * Loaded from templates/toc-script.html at build time.
 */
const TOC_SCRIPT = readTemplate('toc-script.html');
...
/**
 * Return ' class="active"' when the condition is true, empty string otherwise.
 * @param {boolean} isActive
 * @returns {string}
 */
function activeAttr(isActive) {
    return isActive ? ' class="active"' : '';
}

/**
 * Build a single subnav list item.
 * @param {string} href - Link target
 * @param {string} label - Display text (pre-escaped if needed)
 * @param {boolean} isActive - Whether this is the current page
 * @returns {string} HTML <li> element
 */
function buildSubnavItem(href, label, isActive) {
    return `
                <li><a href="${href}"${activeAttr(isActive)}>${label}</a></li>`;
}
...
function buildTripSubmenu(tripMetadata, allContent, currentPage, basePath = './') {
    ...
    html += buildSubnavItem(`${basePath}index.html`, 'Overview', currentPage === 'intro');
    html += buildSubnavItem(`${basePath}route.html`, 'Route', currentPage === 'map');

    articles.forEach(item => {
        const slug = getContentItemSlug(item);
        html += buildSubnavItem(`${basePath}${slug}.html`, escapeHtml(item.title), currentPage === item.title);
    });
...
function buildLocationDropdown(locations, currentPage, basePath) {
    ...
    locations.forEach(loc => {
        ...
        const active = activeAttr(loc.title === currentPage);
        html += `
                        <a href="${basePath}${slug}.html" role="menuitem"${active}>${escapeHtml(loc.title)}</a>`;
    });
...
function generateTripContentPage(tripMetadata, contentItem, allContent, contentIndex, config, domain) {
    ...
    const seoMetadata = generateContentPageMetadata(contentItem, tripMetadata, itemSlug, domain, config.site.title);
