...
import { GALLERY_MARKER, MARKDOWN_IMAGE_REGEX } from './constants.js';
import { convertMarkdown, convertMarkdownString } from './markdown-converter.js';
import { parseNutshellBlock } from './nutshell.js';
...
/**
 * Discover trips with optional filtering.
 * Convenience wrapper around discoverAllTrips that handles filtering logic.
 *
 * @param {string|null} tripFilter - Optional trip ID to filter for. If null, returns all trips.
 * @returns {Array<string>} Array of trip IDs (filtered if tripFilter provided)
 */
function discoverTrips(tripFilter = null) {
    const allTrips = discoverAllTrips(CONFIG.TRIPS_DIR, (id) => CONFIG.getTripConfigPath(id));
    return tripFilter ? allTrips.filter(id => id === tripFilter) : allTrips;
}

/**
 * Strip markdown syntax to produce plain text.
 * Removes images, links, headers, emphasis, horizontal rules, blockquotes, and HTML.
 * Collapses all whitespace to single spaces — suitable for word counts and readability analysis.
 *
...
/**
 * Strip inline markdown for line-level analysis.
 * Unlike stripMarkdownToPlainText, preserves whitespace structure and handles
 * escaped characters — suitable for sentence-level checks in the content audit.
 *
 * @param {string} text - Markdown text (typically a single line or paragraph)
 * @returns {string} Plain text with inline formatting removed
 */
 ...
 * @returns {number} Word count (0 for empty/null input)
 */
 ...
 *
 * @param {string} text - Plain text
 * @returns {number} Sentence count (0 for empty/null input)
 */
 ...
 */
function processMarkdownWithGallery(filePath) {
 ...
 */
async function convertMarkdownWithGallery(filePath) {
    const { markdownContent, galleryImages } = processMarkdownWithGallery(filePath);

    // Extract structured nutshell metadata (duration, verdict) before HTML conversion
    const nutshellData = parseNutshellBlock(markdownContent);

    let html;
    if (galleryImages) {
        // Marker was found — use stripped content (even if no images after it)
        html = convertMarkdownString(markdownContent, path.dirname(filePath));
    } else {
        html = await convertMarkdown(filePath);
    }

    return { html, galleryImages, nutshellData };
}
...
function extractTripMetadata(trip) {
    return {
        ...
        mapNarrative: trip.mapNarrative || '',
        content: trip.content,
...
function generateAndPromoteHomepage(output, domain, generateHomepageFn) {
    ...
    writeTextFile('index.html.new', homepageHtml);
...
function generateMapPageToFile(output, domain, generateMapPageFn) {
    ...
    writeTextFile('map/index.html', mapHtml);
...
function generateSitemapToFile(trips, domain, generateSitemapFn) {
    ...
    writeTextFile('sitemap.xml', sitemapXml);
