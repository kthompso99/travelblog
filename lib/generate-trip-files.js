...
import { generateTripIntroPage, generateTripContentPage } from './generate-trip-pages.js';
import { generateTripMapPage } from './generate-trip-map-page.js';
import { ensureDir, extractTripMetadata, writeTextFile } from './build-utilities.js';
import { getContentItemSlug } from './slug-utilities.js';
import CONFIG from './config-paths.js';

/**
 * Generate intro (index.html) and map (route.html) pages for a trip
 */
function generateIntroAndMapPages(tripMetadata, allContent, output, domain, tripDir, indent) {
    ...
    writeTextFile(introPath, introHtml);
    console.log(`${indent}✅ Intro page → ${tripDir}/index.html`);

    const mapHtml = generateTripMapPage(tripMetadata, allContent, output, domain);
    const mapPath = path.join(tripDir, 'route.html');
    writeTextFile(mapPath, mapHtml);
    console.log(`${indent}✅ Map page → ${tripDir}/route.html`);
...
function copyTripImages(tripId, tripDir, indent) {
    ...
    const imgDst = path.join(tripDir, CONFIG.TRIP_IMAGES_SUBDIR);
