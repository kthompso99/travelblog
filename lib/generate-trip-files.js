...
const { generateTripIntroPage, generateTripContentPage, generateTripMapPage } = require('./generate-html');
const { ensureDir } = require('./build-utilities');
...
function generateTripFiles(tripData, output, domain, indent = '') {
    ...
    locations.forEach((location) => {
        ...
        const locationSlug = location.file.replace('.md', '');
        // Find correct index in allContent (not just in locations array)
        const contentIndex = allContent.findIndex(item => item.title === location.title);
        const locationHtml = generateTripContentPage(tripMetadata, location, allContent, contentIndex, output, domain);
    ...
    articles.forEach((article) => {
        // Use filename (without .md) as slug for stable URLs
        const articleSlug = article.file.replace('.md', '');
        // Find correct index in allContent (not just in articles array)
        const contentIndex = allContent.findIndex(item => item.title === article.title);
        const articleHtml = generateTripContentPage(tripMetadata, article, allContent, contentIndex, output, domain);
