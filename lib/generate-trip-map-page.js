 ...
 * Builds the per-trip map page (route.html) with sidebar, playback controls, and Google Maps
 */

import { generateTripMetadata, escapeHtml } from './seo-metadata.js';
import { getContentItemSlug } from './slug-utilities.js';
import { getTripImagePath } from './image-utilities.js';
import { getGoogleMapsApiKey, getMapStylesJson } from './maps-config.js';
import { readTemplate, renderPage } from './generate-html-helpers.js';
import { fillTemplate } from './template-utilities.js';
import { buildCommentPageId, buildCommentsSection } from './remark42-config.js';
import { buildTripSubmenu, buildTripHero } from './generate-trip-pages.js';
...
/**
 * Generate trip map page (standalone route.html for each trip)
...
function generateTripMapPage(tripMetadata, allContent, config, domain) {
    ...
    // Build narrative paragraph (optional — degrades gracefully)
    const narrativeHtml = tripMetadata.mapNarrative
        ? `<p class="map-narrative">${escapeHtml(tripMetadata.mapNarrative)}</p>`
        : '';
